/**
 * Client -> server command contracts.
 *
 * Mirrors `backend/src/websocket/protocol/clientEvents.ts` exactly.
 * `GameMode` is a string-literal union mirroring the backend `GameMode` enum
 * values (the frontend does not use TS enums).
 */
