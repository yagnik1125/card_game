...
import { connect, disconnect, emitWithAck } from "@/ws/client/socketClient";
import { getConnectionStatus } from "@/ws/client/connection";
...
export default function WsHomePage() {
    ...
    const create = async () => {
        ...
        try {
            setCreating(true);
            setCreateError(null);
            const ack = await emitWithAck<{ gameId: string }>("GAME:CREATE", {
                numberOfRounds: rounds,
                difficulty,
                mode,
            });
            if (!ack.ok || !ack.data?.gameId) {
                throw new Error(
                    ack.error?.message ?? "Failed to create game. Please try again."
                );
            }
            await wait(1000);
            navigate(
                mode === "TEAMS_2V2"
                    ? `/ws/game/team2v2/${ack.data.gameId}`
                    : `/ws/game/${ack.data.gameId}`
            );
        }
        catch (error) {
    ...
    const runHealthCheck = () => {
        connect();
        // Probe the server directly: if it is down the ack times out and we
        // surface the error UI instead of spinning forever.
        emitWithAck("GAME:PING", undefined, 3000)
            .then((ack) => {
                if (ack.ok && getConnectionStatus() === "connected") {
                    setHealthy(true);
                    ...
                    setHealthy(false);
                    setConnectionError(
                        ack.error?.message ?? "Cannot reach server"
                    );
                }
            })
            .catch((error) => {
    ...
    useEffect(() => {
        runHealthCheck();
        // Release the shared socket reference so the singleton can fully tear
        // down when this page unmounts (the game pages own their own refs).
        return () => disconnect();
    }, []);
