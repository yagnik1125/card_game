...
export class HardBot implements BotStrategy {
  chooseCard(
    ...
    const currentWinningPlay =
      this.getCurrentWinningPlay(
        trick,
        trumpSuit
      );
    const winningCards =
      legalCards.filter(card =>
        this.canBeat(
          card,
          currentWinningPlay.card,
          trick.leadSuit!,
          trumpSuit
        )
      );
    if (winningCards.length > 0) {
      const cheapestWinningCard =
        winningCards.sort(
          (a, b) => a.rank - b.rank
        )[0];
      return {
  ...
  private canBeat(
    ...
    if (
      challenger.suit === currentWinner.suit
    ) {
      return (
        challenger.rank >
        currentWinner.rank
